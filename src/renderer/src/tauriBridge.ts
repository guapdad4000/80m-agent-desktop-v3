...
export const installTauriBridge = (): void => {
  ...
  window.electron = {
    process: {
      platform:
        navigator.userAgent.includes("Mac") ? "darwin" : navigator.platform,
      versions: {},
    },
  ...
  window.hermesAPI = {
    ...
    runHermesUpdate: () =>
      call("run_hermes_update", {}, {
        success: false,
        error: fallbackNotice("run_hermes_update"),
      }),

    checkOpenClaw: () => call("check_open_claw", {}, { found: false, path: null }),
    runClawMigrate: () =>
      call("run_claw_migrate", {}, {
        success: false,
    ...
    setEnv: (key, value, profile) => call("set_env", { key, value, profile }, false),
    ...
    getHermesHealth: (profile) =>
      call("get_hermes_health", { profile }, {
        install: {
          installed: false,
          configured: false,
          hasApiKey: false,
    ...
    abortChat: () => call("abort_chat", {}, undefined),
    ...
    addMemoryEntry: (content, profile) =>
      call("add_memory_entry", { content, profile }, {
        success: false,
        error: fallbackNotice("add_memory_entry"),
      }),
    updateMemoryEntry: (index, content, profile) =>
      call("update_memory_entry", { index, content, profile }, {
        success: false,
    ...
    writeUserProfile: (content, profile) =>
      call("write_user_profile", { content, profile }, {
        success: false,
        error: fallbackNotice("write_user_profile"),
      }),

    readSoul: (profile) => call("read_soul", { profile }, ""),
    writeSoul: (content, profile) => call("write_soul", { content, profile }, false),
    ...
    getSkillContent: (skillPath) => call("get_skill_content", { skillPath }, ""),
    installSkill: (identifier, profile) =>
      call("install_skill", { identifier, profile }, {
        success: false,
        error: fallbackNotice("install_skill"),
      }),
    uninstallSkill: (name, profile) =>
      call("uninstall_skill", { name, profile }, {
        success: false,
    ...
    searchSessions: (query, limit) => call("search_sessions", { query, limit }, []),
    ...
    addModel: (name, provider, model, baseUrl) =>
      call("add_model", { name, provider, model, baseUrl }, {
        id: "",
        name,
        provider,
        model,
    ...
    onMenuSearchSessions: (callback) => onEvent("menu-search-sessions", callback),
    ...
    runHermesBackup: (profile) =>
      call("run_hermes_backup", { profile }, {
        success: false,
        error: fallbackNotice("run_hermes_backup"),
      }),
    runHermesImport: (archivePath, profile) =>
      call("run_hermes_import", { archivePath, profile }, {
        success: false,
        error: fallbackNotice("run_hermes_import"),
      }),
    runHermesDump: () => call("run_hermes_dump", {}, fallbackNotice("run_hermes_dump")),
    ...
    onPlaywrightNavigated: (callback) => onEvent("playwright-navigated", callback),
  };
};
